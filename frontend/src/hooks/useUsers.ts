...
interface UseUsersReturn extends UsersState {
  ...
  handleCreate: (data: CreateUserInput) => Promise<UserResponse>;
  handleUpdate: (id: string, data: UpdateUserInput) => Promise<UserResponse>;
  handleDelete: (id: string) => Promise<DeleteUserResponse>;
}
...
export const useUsers = (): UseUsersReturn => {
  ...
  //Return full response so callers can access backend message

  const handleCreate = async (data: CreateUserInput): Promise<UserResponse> => {
  ...
  const handleDelete = async (id: string): Promise<DeleteUserResponse> => {
